...
@interface DuanZiViewController ()<EGORefreshTableHeaderDelegate,UIGestureRecognizerDelegate>
...
- (void)viewDidLoad
{
    ...
    if ([self.serverUrl length]<=0) {
         [persistentDefaults setObject:@"http://121.199.45.245/butubukuai" forKey:@"ServerUrl"];
    }
   //   http://121.199.45.245/butubukuai
    ...
    //初始化url
    self.urlString=[NSString stringWithFormat:@"%@/NeiHanArticle/",self.serverUrl];
            ...
            for (id articleclassname in deserializedDictionary) {
                ...
                if ([articleclassname isEqualToString:@"results"]) {   //获得具体内涵段子信息，接着进行解析
                   ...
                   // id nextArticelUrl=[deserializedDictionary objectForKey:articleclassname];
                    NSString *nextUrl=[NSString stringWithFormat:@"%@/NeiHanArticle/?page=%d",self.serverUrl,++self.page];
...
-(User *) getUserNameById:(NSNumber *)userid{
    ...
    NSString *urlAsString = [NSString stringWithFormat:@"%@/user/%@/",self.serverUrl,userid];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row%3==0) {
        ...
        if ([nowDuanZi.userInfo.headerimage length]>0) {
             [headerCell setUrlByString:[NSString stringWithFormat:@"%@/media/%@",self.serverUrl,nowDuanZi.userInfo.headerimage]];
        }
        ...
        [middleCell.myTextLabel setFrame:CGRectMake(CELL_CONTENT_MARGIN, CELL_CONTENT_MARGIN, CELL_CONTENT_WIDTH - (CELL_CONTENT_MARGIN * 2), MAX(size.height, 44.0f))];  //设置label的fram
...
- (IBAction)praiseBtnClick:(id)sender {
    ...
    if ([clickDuanzi.ifAdded boolValue]==NO) {
        if ([self isConnectionAvailable]) {
            ...
            //向服务器发送好评的MD5code
            NSString *urlAsString = [NSString stringWithFormat:@"%@/addNaiHanArticlePraiseNumber/md5code=%@",self.serverUrl,md5code];
         ...
         [self alertMessageClicled:cell andShowStartWidth:52 andMessage:@"已点击过"]; //弹出已点击按钮
    }
...
- (IBAction)badBtnClick:(id)sender {
    ...
    if ([clickDuanzi.ifAdded boolValue]==NO) {
        if ([self isConnectionAvailable]) {
            //向服务器发送好评的MD5code
            NSString *urlAsString = [NSString stringWithFormat:@"%@/addNaiHanArticleBadNumber/md5code=%@",self.serverUrl,md5code];
...
- (IBAction)collecteBtnCilck:(id)sender {
    ...
    if ([name length]>0) { //已登录
        if ([clickDuanZi.ifCollected boolValue]==NO) {
            ...
            if ([self isConnectionAvailable]) {  //判断网络连接
                NSString *urlAsString = [NSString stringWithFormat:@"%@/addCollection/collector=%@&collectiontype=%@&pid=%@&pmd5code=%@&content=%@&imageurl=%@",self.serverUrl,name,@"duanzi",clickDuanZi.idnumber,clickDuanZi.md5code,clickDuanZi.article,nil];
                ...
                [req addValue:@"text/xml; charset=utf-8" forHTTPHeaderField:@"Content-Type"];
                NSError *error=nil;
                NSData *condata=[NSURLConnection sendSynchronousRequest:req returningResponse:nil error:&error];  //先服务器发送消息
                if ([condata length]>0&& error == nil) {  //服务器返回数据，收藏成功
...
- (void) prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    ...
    if([[segue identifier] isEqualToString:@"commentsegue"]){
        ...
        if ([controller isKindOfClass:[CommentViewController class]]) {
            ...
            [commentViewController getPmd5code:nowDuanZi.md5code andFrom:@"DuanZi"];
        } else {
        ...
        if ([controller isKindOfClass:[EditViewController class]]) {
            ...
            [editViewController sendFrom:@"duanzi"];
